...
Once you are all set up we can create our first test. By convention test files have the same name as the component being tested with the .test extension such as *example.test.js*
...
The [expect](https://jestjs.io/docs/expect#expect) function allows us to test a value. Typically expect will be used with a ["matcher" function](https://jestjs.io/docs/expect#matchers) to assert something about a value. In this case we *expected* that string to be in the DOM.  
 There are plenty of *matcher functions* that we can use with the expect method. You can take a look at them by following the above link.

You can run *yarn test* or *npm test* to run your test.
